...
agent1 = 'HybridAgent_V2'
agent2 = 'HybridAgent_V2'
...
class HybridAgent_V2(CaptureAgent):
    ...
    Affectionately called "V2", we were not able to consistently beat this agent if he made up a full team.
    """
        ...
        for x in range(width):
            for y in range(height):
                if not game_state.has_wall(x, y):
                    ...
                    if game_state.has_wall(x, y-1): neighbors += 1
                    # cell with 3 walls is the tip of a dead end
                    if neighbors >= 3:
    ...
    def choose_action(self, game_state):
        ...
        carry_limit = 6  # greedy limit (collect more before returning) - too high? - testing! RE: seems to work fine
        ...
        # A* search! 
        # lower node limit when fleeing forr instant reaction
...
class HybridAgent(CaptureAgent):
    """
    Our original HybridAgent which scored 2nd & 3rd on the test competitions. A lot of this code and the concepts are
    reused in V2.
    """
    def register_initial_state(self, game_state):
        CaptureAgent.register_initial_state(self, game_state)
    ...
    def choose_action(self, game_state):
        ...
        # 3. EXECUTE A* (with anti dance / loop logic )
        next_action = self.a_star_search(game_state, my_pos, target_goals, dangerous_ghosts)
        
        if next_action:
            return next_action
        
        # 4. PANIC FALLBACK (when search returns None, don't just STOP)
        # pick random move that doesn't kill us
